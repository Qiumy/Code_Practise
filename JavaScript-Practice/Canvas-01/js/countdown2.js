...
//const endTime = new Date(2016,0,20,18,47,52);
...
window.onload = function() {
	...
	clock.width = WINDOW_WIDTH;
	clock.height = WINDOW_HEIGHT;
	...
	for(var i=0; i<digit[num].length; i++)
		for(var j=0; j<digit[num][i].length; j++)
			if (digit[num][i][j]==1) {
				...
				cxt.arc(x+j*2*(RADIUS+1)+(RADIUS+1),y+i*2*(RADIUS+1)+(RADIUS+1),RADIUS,
					0,2*Math.PI);
				cxt.closePath();
				cxt.fill();
			}
